...
const headers = computed(() => [
  { title: t('user'), key: 'new_user', sortable: false },
...
const fetchReferredUsers = async (page = 1) => {
  ...
  try {
    ...
    const response = await axios.get('/affiliate/referrals/referred-users', { params })
        ...
        >
          ...
          <template #no-data>
            <div class="text-center py-8">
              ...
              <h6 class="text-h6 mb-2">{{ t('no_referred_users_yet') }}</h6>
              <p class="text-body-2">{{ t('no_referred_users_yet_description') }}</p>
            </div>
          </template>
        </VDataTableServer>
      </VCardText>
